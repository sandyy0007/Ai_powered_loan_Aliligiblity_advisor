...
def prediction_page():
    ...
    if st.checkbox("Show real-time prediction as you adjust values"):
        ...
        preview_prediction = model.predict([features])
        if preview_prediction == "N":
            st.warning("⚠️ Based on current inputs, loan is likely to be **Rejected**")
        else:
            st.success("✅ Based on current inputs, loan is likely to be **Approved**")
    ...
    if st.button("🔮 Predict Loan Status"):
        ...
        if prediction == "N":
            ...
            *Take care to address these issues before trying again!*
            ...
            # Financial insights chart for rejection
            ...
            **Key highlights of your application:**
            ...
            # Financial insights chart for approval
            st.markdown("### 📊 Financial Insights")
            # Income vs Loan Amount comparison
            fig, ax = plt.subplots()
            ax.bar(['Applicant Income', 'Coapplicant Income', 'Loan Amount'], [ApplicantIncome, CoapplicantIncome, LoanAmount], color=['blue', 'green', 'red'])
            ax.set_ylabel('Amount ($)')
            ax.set_title('Income vs Loan Amount Comparison')
            st.pyplot(fig)
...
def sidebar_layout():
    ...
    # Add mobile responsiveness note
    st.sidebar.markdown("---")
    st.sidebar.markdown("📱 **Mobile Friendly**: This app is optimized for mobile devices.")
